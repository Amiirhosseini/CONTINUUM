...
In early 2026 I observed long running agents failing on recovery, not reasoning. Checkpoints were treated as proof to continue, not evidence to verify. An agent could certify its own progress and be believed. A tool could fire twice after a restore and no one would notice until a customer was billed twice. Surveying durable execution in Temporal and Restate, checkpointing in LangGraph, rollback hazards in ACRFence 2603.20625, error tail degradation in self conditioning 2509.09677, and horizon limits in METR, I found the gap was not another framework. It was a portable verification substrate that asks, given the state at time T and the world as it is now, is it still correct and safe to continue.

Over three weeks I designed and built CONTINUUM from one invariant: every fact carries its origin and trust is earned. The substrate is a hash chained log with `verify() trusted_through`, a ledger with stable key deduplication and token fallback, a gate and gateway that block unclaimed effects before they fire, and a recovery engine that propagates staleness `dependency -> evidence -> finding -> decision` and seals a contract with `safe`, `evidence`, and `human_steps`. Five seams expose the same log to Claude Code, LangGraph, LangChain, OpenAI, HTTP, and OpenTelemetry. I validated it with real kills `os._exit(9)` and `os._exit(137)`, three Claude Code e2e runs at 7/7, OpenRouter `gpt-4o-mini` hard crash proofs, and a benchmark that prints `0 duplicates` where naive replay prints `50`. It holds about 1380 tests, 38 CLI commands, and 12 MCP tools, all built in public on `main`.
